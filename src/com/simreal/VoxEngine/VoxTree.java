...
public class VoxTree {
    ...
    public int pickNodeParentIndex;
    ...
    public void setPickedVoxel(int color){
        if (pickNodeIndex > 0){
            System.out.println("Pick "+ pickNodeIndex);
            nodeArray[pickNodeIndex] = Node.setColor(nodeArray[pickNodeIndex], color);
        }
    }

    public void setPickedParentVoxel(int color){
        if (pickNodeIndex > 0){
            System.out.println("Pick parent "+ pickNodeParentIndex);
            long node = nodeArray[pickNodeParentIndex];
            node = Node.setLeaf(node, true);
            node = Node.setColor(node, color);
            //node = Node.setChild(node, 0);
            nodeArray[pickNodeParentIndex] = node;
        }
    }
    ...
    private long castSubtree(Point3d t0, Point3d t1, boolean pick){
        ...
        while (stateStackTop > 0){
            ...
            if (Node.isLeaf(node)) {
                ...
                // TODO: This is NOT the way to set this
                pickNodeParentIndex = state.nodeIndex;
